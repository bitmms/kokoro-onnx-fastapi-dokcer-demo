> **基于 Kokoro 构建的中文 TTS 服务**，参考项目：https://github.com/kamjin3086/kokoro-onnx-fastapi
> * kokoro
> * fastapi
> * uv
> * docker



# 一、从源代码使用 uv 运行

```bash
git clone https://github.com/bitmms/kokoro-onnx-docker-demo.git
```

```bash
cd kokoro-onnx-docker-demo
```

```bash
wget https://github.com/bitmms/kokoro-onnx-fastapi-docker-demo/releases/download/model/models.zip
```

```bash
unzip models.zip
```

```bash
uv sync
```

```bash
uv run main.py
```



# 二、从源代码打包 Docker 镜像运行

```bash
git clone https://github.com/bitmms/kokoro-onnx-fastapi-docker-demo.git
```

```bash
cd kokoro-onnx-fastapi-docker-demo
```

```bash
wget https://github.com/bitmms/kokoro-onnx-fastapi-docker-demo/releases/download/model/models.zip
```

```bash
unzip models.zip
```

```bash
docker build -t kokoro-onnx-fastapi-docker-demo:latest .
```

```bash
docker run -d --name kokoro-onnx-fastapi-docker-demo -p 8210:8210 kokoro-onnx-fastapi-docker-demo:latest
```

```bash
docker exec -it kokoro-onnx-fastapi-docker-demo /bin/bash
```



# 三、从 Docker 镜像压缩包运行
```bash
docker save kokoro-onnx-fastapi-docker-demo:latest -o kokoro-onnx-fastapi-docker-demo-latest.tar
```

```bash
docker load < kokoro-onnx-fastapi-docker-demo-latest.tar
```

```bash
docker run -d --name kokoro-onnx-fastapi-docker-demo -p 8210:8210 kokoro-onnx-fastapi-docker-demo:latest
```

```bash
docker exec -it kokoro-onnx-fastapi-docker-demo /bin/bash
```



# 四、从 DockerHub 运行

```bash
docker run -d -p 8210:8210 XXXXXXXXXXXXXXXXXXXXX
```



# 五、测试
```bash
curl -X POST "http://127.0.0.1:8210/generate-speech/" \
     -H "Content-Type: application/json" \
     -d '{"text": "今天天气真好，请尽快撤离", "voice": "zm_100", "speed": 1.0}' \
     --output test.wav
```
